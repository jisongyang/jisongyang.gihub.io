...
// 读取单个文件
function read_singe_file(now_novel){
    file_name='novel/'+now_novel+'.txt'
...
function get_result(){
    ...
    for(now_novel in novel_dict){
        ...
        all_content=read_singe_file(now_novel)
        all_chapter=all_content.split('###^###')
        ...
        for(now_chapter of chapter_list){
            // 读取当前章节
            now_content=all_chapter[now_chapter]
